...
! z-move (if requested)
     IF (trace_z_shift) THEN
        ...
        Rate_energy_heat = Rate_energy_heat - KVx * species(s)%part(k)%VZ
     END IF
...
! if crossed the left boundary
        ...
        IF (PeriodicBoundaryFlag.EQ.1) THEN
           Xtmp = N_cells + Xtmp 
           species(s)%part(k)%X = Xtmp !+ N_cells    ! move particle to the right end of the system
...
! for periodic boundaries prevent charge accumulation
     ...
     Q_left = 0
     s=1
     Q_strm_spec(0,s) = Q_strm_spec(0,s) + Q_strm_spec(N_cells,s)
     Q_strm_spec(N_cells,s) = Q_strm_spec(0,s)
     IF (N_spec.EQ.2) THEN
! ions
        s = 2
        Q_strm_spec(0,s) = Q_strm_spec(0,s) + Q_strm_spec(N_cells,s)
  ...
  IF (trace_z_shift) THEN
     ...
     DO k = 1, N_part(s)
        IF (species(s)%part(k)%Z.LT.(-max_Z_shift)) THEN
           ...
           Rate_energy_coll(s) = Rate_energy_coll(s) - (species(s)%part(k)%VX**2 + species(s)%part(k)%VY**2 + species(s)%part(k)%VZ**2)
