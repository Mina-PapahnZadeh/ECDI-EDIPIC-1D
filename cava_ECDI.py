...
kyvec_cyc = np.linspace(0.0,15., 800)
...
def plot_im_re_sep(w):
    ...
    plt.figure(2)
    for i in range(len(kzvec)):
        plt.plot(kyvec_cyc / ( k0*rho_e), np.imag(w[i])*200, lw=2.0)#, label=r"$k_z \lambda_D = %.5f$" % kzvec[i])
    ...
    plt.ylabel(r"$\gamma*200 / \omega_{pi}$", fontsize=20)
...
if __name__ == "__main__":
    main()
#####################################
mode=np.array([28,34,40,46,52,58,64,70,76,82,94,100,106,112])
growth10000=np.array([2.53,3.79,3.46,6.04,5.29,6.14,5.45,5.83,5.48,5.42,5.10,5.15,5.23,4.16])*1e7
n0=2103
k=np.arange(-n0/2,n0/2)*2*np.pi/L
data= open("Ekr60.dat","r")
Ekr    = []
for j in range(n0):
    tempE20 = data.readline().split()
    Ek = float(tempE20[0])
    Ekr.append(Ek)

Ekr = np.array(Ekr)
plt.plot(k/k0,Ekr*(2/n0))
plt.xlim((0, 12))
#plt.plot(mode,growth10000,'k s',markersize=5,label='$\gamma_{simula}-B=100\;G$')
####################################################################
...
def plot_im_re_sep(w):
    ...
    for i in range(len(kzvec)):
        plt.plot(kyvec_cyc / (k0* rho_e), np.imag(w[i])*200, '.',color='red', lw=2.0,label=r'$\gamma$-theory')
...
#mode=np.array([28,29,30,34,35,40,46,52,58,64,70,76,82,94,100,106,112])
#mode=np.array([29,30,31,32,50,51,52,53,54,55,56,75,76,77,78,79])
#growth10000=np.array([2.53,2.3,4.02,3.79,1.65,3.46,6.04,5.29,6.14,5.45,5.83,5.48,5.42,5.10,5.15,5.23,4.16])*1e7
...
#plt.plot(mode,growth10000,'k s',markersize=5,label='$\gamma-B=100\;G$')
